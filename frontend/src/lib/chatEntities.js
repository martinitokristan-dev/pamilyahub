...
const EXPENSE_WEAK_REASONS = new Set([
  'expense',
  'today',
  'yesterday',
  'now',
...
/** True when the message is only a plan-creation trigger, not an actual plan name. */
export function isBarePlanTrigger(text = '') {
  const n = normalizeText(text)
  return Boolean(n && BARE_PLAN_TRIGGER_RX.test(n))
}

/** Strip filler / wallet / time words so slot-filling continues instead of auto-logging. */
export function sanitizeExpenseReason(reason) {
  if (!reason) return null
  const lower = String(reason).toLowerCase().trim()
  if (!lower || EXPENSE_WEAK_REASONS.has(lower)) return null
  return reason
}

/** Strip bare plan trigger words so "add a new plan" starts the name slot. */
export function sanitizePlanReason(reason, sourceText = '') {
  if (!reason) return null
...
export function extractPersonName(text = '') {
  ...
  const cleaned = normalized
    .replace(/\b(i owe|owes me|lent|borrowed|utang ko|utang nako|nagbayad|bayad|pay|paid|debt|partial|mark paid)\b/g, ' ')
    .replace(/(?:₱|php|peso|pesos)?\s*\d[\d,]*\.?\d*/g, ' ')
    .replace(/\b(from|to|kay|kang|ni|sa|for|me|my|i|ako|nako|ko|si|ang|money|cash|wallet|accounts)\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
  ...
  const exclude = [
    ...
    'nagbayad', 'binayaran', 'nagpautang', 'umutang', 'nangutang'
